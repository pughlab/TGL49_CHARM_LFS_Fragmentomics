...
Data <- Data[, names]
Data_validation <- Data[c(samples_hbc_val$sWGS, samples_neg_val$sWGS), ]
Data_validation <- Data_validation[complete.cases(Data_validation), ]

Data <- Data[c(samples_hbc_test$sWGS, samples_neg_test$sWGS), ]
Data$sample <- row.names(Data)
Data <- Data[complete.cases(Data), ]

y <- c(rep("negative", sum(row.names(Data) %in% samples_hbc_test$sWGS)))
y <- c(y, rep("positive", sum(!(row.names(Data) %in% samples_hbc_test$sWGS))))
...
saveRDS(outcomes_neg_test, file.path(outdir, "outcomes_integrated_status_test.rds"))
saveRDS(outcomes_neg_val, file.path(outdir, "outcomes_integrated_status_val.rds"))
