...
samples_de <- as.data.frame(cbind(data_samples[, c("sWGS", "mutation_type")], "LFS"))
...
colnames(de_table) <- c("feature", "cohort", "c2", "c3", "lof", "splice", "padj_cohort", "padj_c2", 
                        "padj_c3", "padj_lof", "padj_splice", "healthy_median", "healthy_sd")
write.table(de_table, file.path(outdir, "fragment_nucleosome_TCGA_differential.txt"), sep = "\t", row.names = FALSE)
...
data_means_cov <- as.matrix(data_means[grepl("coverage", row.names(data_means)), ])
...
right_annotation2 <- rowAnnotation("Means" = data_means_cov,
...
right_annotation3 <- rowAnnotation("Means" = data_means_amp,
                                   show_annotation_name = FALSE,
                                   border = FALSE,
                                   col = list("Means" = col_means),
                                   annotation_name_gp = gpar(fontsize = 8),
...
Amplitude <- Heatmap(data_amp,
                     ...
                     show_column_names = FALSE,
                     show_heatmap_legend = FALSE,
                     right_annotation = right_annotation3,
                     column_title_gp = gpar(fontsize = 8),
                     row_labels = NULL,
                     row_title = "Amplitude",
                     row_title_gp = gpar(fontsize = 8),
...
pdf(file.path(outdir, "fragment_nucleosome_TCGA_clustering_expanded.pdf"), height = 5, width = 6)
Heatmap <- Midpoint %v% Coverage %v% Amplitude
