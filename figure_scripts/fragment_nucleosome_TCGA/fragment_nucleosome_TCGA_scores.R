...
data_melt$color <- ifelse(data_melt$cancer_status == "Positive" & data_melt$color == "LFS", "Other Cancer", data_melt$color)
data_melt$color <- factor(data_melt$color, levels = c("HBC", "LFS", "Matched Cancer", "Other Cancer"),
                          labels = c("Healthy Control", "LFS Cancer Negative", "LFS Cancer Positive (Matched)", "LFS Cancer Positive (Other)"))
...
ggsave(file.path(outdir, paste0("fragment_nucleosome_TCGA_scores.pdf")), site_plot, device = "pdf", width = 12, height = 4, units = "in", limitsize = FALSE)

### Plot shorter version
sites <- c("ACC", "BLCA", "BRCA", "GBM", "LGG", "LUAD", "LUSC", "PRAD", "SKCM")
...
ggsave(file.path(outdir, paste0("fragment_nucleosome_TCGA_scores_short.pdf")), site_plot2, device = "pdf", width = 8, height = 5, units = "in", limitsize = FALSE)
