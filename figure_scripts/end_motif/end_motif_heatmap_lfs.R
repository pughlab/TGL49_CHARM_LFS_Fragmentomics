...
samples <- read.delim("/Users/derekwong/OneDrive - UHN/Post-Doc/CHARM_Project/LFS/samples/sample_list.txt")
...
samples <- samples[!(samples$sWGS %in% c(exclude, abnormal)), ]

### Calculate Previvor Median
samples_pre <- samples[samples$cancer_status == "negative" & samples$previous_cancer == "no", ]
data_pre <- data[, colnames(data) %in% samples_pre$sWGS]
pre_median <- rowMedians(as.matrix(data_pre))
...
## Generate Heatmap
pdf(file.path(outdir, "end_motifs_heatmap_lfs.pdf"), height = 7, width = 5)
